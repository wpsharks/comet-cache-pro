			...
			public function options()
				{
					...
					echo '      <p style="margin-bottom:0;"><textarea name="'.esc_attr(__NAMESPACE__).'[save_options][cache_clear_eval_code]" rows="5" spellcheck="false" class="monospace">'.format_to_edit(plugin()->options['cache_clear_eval_code']).'</textarea></p>'."\n";
					...
					echo '            <option value="1"'.selected(plugin()->options['cache_purge_term_post_tag_enable'], '1', FALSE).'>'.__('Yes, if any single Post/Page is purged/reset; also purge the associated Tag archive views.', plugin()->text_domain).'</option>'."\n";
					echo '            <option value="0"'.selected(plugin()->options['cache_purge_term_post_tag_enable'], '0', FALSE).'>'.__('No, my site doesn\'t use Tags and/or I don\'t have any Tag archive views.', plugin()->text_domain).'</option>'."\n";
					echo '         </select></p>'."\n";
					...
					echo '            <option value="0"'.selected(plugin()->options['when_logged_in'], '0', FALSE).'>'.__('No, do NOT cache; or serve a cache file when a user is logged-in (safest option).', plugin()->text_domain).'</option>'."\n";
					echo '            <option value="postload"'.selected(plugin()->options['when_logged_in'], 'postload', FALSE).'>'.__('Yes, and maintain a separate cache for each user (recommended for membership sites).', plugin()->text_domain).'</option>'."\n";
					echo '            <option value="1"'.selected(plugin()->options['when_logged_in'], '1', FALSE).'>'.__('Yes, but DON\'T maintain a separate cache for each user (I know what I\'m doing).', plugin()->text_domain).'</option>'."\n";
					echo '         </select></p>'."\n";
					...
					echo '      <p><textarea name="'.esc_attr(__NAMESPACE__).'[save_options][exclude_uris]" rows="5" spellcheck="false" class="monospace">'.format_to_edit(plugin()->options['exclude_uris']).'</textarea></p>'."\n";
					...
					echo '      <p><textarea name="'.esc_attr(__NAMESPACE__).'[save_options][exclude_refs]" rows="5" spellcheck="false" class="monospace">'.format_to_edit(plugin()->options['exclude_refs']).'</textarea></p>'."\n";
					...
					echo '      <p><textarea name="'.esc_attr(__NAMESPACE__).'[save_options][exclude_agents]" rows="5" spellcheck="false" class="monospace">'.format_to_edit(plugin()->options['exclude_agents']).'</textarea></p>'."\n";
					...
					echo '            <option value="0"'.selected(plugin()->options['auto_cache_enable'], '0', FALSE).'>'.__('No, leave the Auto-Cache Engine disabled please.', plugin()->text_domain).'</option>'."\n";
					echo '            <option value="1"'.selected(plugin()->options['auto_cache_enable'], '1', FALSE).'>'.__('Yes, I want the Auto-Cache Engine to keep pages cached automatically.', plugin()->text_domain).'</option>'."\n";
					echo '         </select></p>'."\n";
					echo '      <hr />'."\n";
					echo '      <div class="plugin-menu-page-panel-if-enabled">'."\n";
					echo '         <h3>'.__('XML Sitemap URL (or an XML Sitemap Index)', plugin()->text_domain).'</h3>'."\n";
					echo '         <table style="width:100%;"><tr><td style="width:1px; font-weight:bold; white-space:nowrap;">'.esc_html(site_url('/')).'</td><td><input type="text" name="'.esc_attr(__NAMESPACE__).'[save_options][auto_cache_sitemap_url]" value="'.esc_attr(plugin()->options['auto_cache_sitemap_url']).'" /></td></tr></table>'."\n";
					echo '         <h3>'.__('A List of URLs to Auto-Cache (One Per Line)', plugin()->text_domain).'</h3>'."\n";
					echo '         <p><textarea name="'.esc_attr(__NAMESPACE__).'[save_options][auto_cache_other_urls]" rows="5" spellcheck="false" class="monospace">'.format_to_edit(plugin()->options['auto_cache_other_urls']).'</textarea></p>'."\n";
					echo '         <hr />'."\n";
					echo '         <h3>'.__('Auto-Cache User-Agent String', plugin()->text_domain).'</h3>'."\n";
					echo '         <table style="width:100%;"><tr><td><input type="text" name="'.esc_attr(__NAMESPACE__).'[save_options][auto_cache_user_agent]" value="'.esc_attr(plugin()->options['auto_cache_user_agent']).'" /></td><td style="width:1px; font-weight:bold; white-space:nowrap;">; '.esc_html(__NAMESPACE__.' '.plugin()->version).'</td></tr></table>'."\n";
					...
					echo '            <option value="0"'.selected(plugin()->options['htmlc_enable'], '0', FALSE).'>'.__('No, do NOT compress HTML/CSS/JS code at runtime.', plugin()->text_domain).'</option>'."\n";
					echo '            <option value="1"'.selected(plugin()->options['htmlc_enable'], '1', FALSE).'>'.__('Yes, I want to compress HTML/CSS/JS for blazing fast speeds.', plugin()->text_domain).'</option>'."\n";
					echo '         </select></p>'."\n";
					...
					echo '               <option value="1"'.selected(plugin()->options['htmlc_compress_combine_head_body_css'], '1', FALSE).'>'.__('Yes, combine CSS from &lt;head&gt; and &lt;body&gt; into fewer files.', plugin()->text_domain).'</option>'."\n";
					echo '               <option value="0"'.selected(plugin()->options['htmlc_compress_combine_head_body_css'], '0', FALSE).'>'.__('No, do not combine CSS from &lt;head&gt; and &lt;body&gt; into fewer files.', plugin()->text_domain).'</option>'."\n";
					echo '            </select></p>'."\n";
					echo '         <p><select name="'.esc_attr(__NAMESPACE__).'[save_options][htmlc_compress_css_code]" autocomplete="off">'."\n";
					echo '               <option value="1"'.selected(plugin()->options['htmlc_compress_css_code'], '1', FALSE).'>'.__('Yes, compress the code in any unified CSS files.', plugin()->text_domain).'</option>'."\n";
					echo '               <option value="0"'.selected(plugin()->options['htmlc_compress_css_code'], '0', FALSE).'>'.__('No, do not compress the code in any unified CSS files.', plugin()->text_domain).'</option>'."\n";
					echo '            </select></p>'."\n";
					echo '         <p><select name="'.esc_attr(__NAMESPACE__).'[save_options][htmlc_compress_combine_head_js]" autocomplete="off">'."\n";
					echo '               <option value="1"'.selected(plugin()->options['htmlc_compress_combine_head_js'], '1', FALSE).'>'.__('Yes, combine JS from &lt;head&gt; into fewer files.', plugin()->text_domain).'</option>'."\n";
					echo '               <option value="0"'.selected(plugin()->options['htmlc_compress_combine_head_js'], '0', FALSE).'>'.__('No, do not combine JS from &lt;head&gt; into fewer files.', plugin()->text_domain).'</option>'."\n";
					echo '            </select></p>'."\n";
					...
					echo '               <option value="1"'.selected(plugin()->options['htmlc_compress_inline_js_code'], '1', FALSE).'>'.__('Yes, compress inline JavaScript snippets.', plugin()->text_domain).'</option>'."\n";
					echo '               <option value="0"'.selected(plugin()->options['htmlc_compress_inline_js_code'], '0', FALSE).'>'.__('No, do not compress inline JavaScript snippets.', plugin()->text_domain).'</option>'."\n";
					echo '            </select></p>'."\n";
					echo '         <p><select name="'.esc_attr(__NAMESPACE__).'[save_options][htmlc_compress_html_code]" autocomplete="off">'."\n";
					echo '               <option value="1"'.selected(plugin()->options['htmlc_compress_html_code'], '1', FALSE).'>'.__('Yes, compress (remove extra whitespace) in the final HTML code too.', plugin()->text_domain).'</option>'."\n";
					echo '               <option value="0"'.selected(plugin()->options['htmlc_compress_html_code'], '0', FALSE).'>'.__('No, do not compress the final HTML code.', plugin()->text_domain).'</option>'."\n";
					echo '            </select></p>'."\n";
					echo '         <hr />'."\n";
					...
					echo '         <p><textarea name="'.esc_attr(__NAMESPACE__).'[save_options][htmlc_css_exclusions]" rows="5" spellcheck="false" class="monospace">'.format_to_edit(plugin()->options['htmlc_css_exclusions']).'</textarea></p>'."\n";
					...
					echo '         <p><textarea name="'.esc_attr(__NAMESPACE__).'[save_options][htmlc_js_exclusions]" rows="5" spellcheck="false" class="monospace">'.format_to_edit(plugin()->options['htmlc_js_exclusions']).'</textarea></p>'."\n";
					echo '         <p class="info" style="display:block;">'.__('<strong>Note:</strong> please remember that your entries here should be formatted as a line-delimited list; e.g. one exclusion pattern per line.', plugin()->text_domain).'</p>'."\n";
					echo '         <hr />'."\n";
					echo '         <h3>'.__('Public HTML Compression Cache Directory', plugin()->text_domain).'</h3>'."\n";
					echo '         <table style="width:100%;"><tr><td style="width:1px; font-weight:bold; white-space:nowrap;">'.esc_html(ABSPATH).'</td><td><input type="text" name="'.esc_attr(__NAMESPACE__).'[save_options][htmlc_cache_dir_public]" value="'.esc_attr(plugin()->options['htmlc_cache_dir_public']).'" /></td><td style="width:1px; font-weight:bold; white-space:nowrap;">/</td></tr></table>'."\n";
					...
					echo '         <table style="width:100%;"><tr><td style="width:1px; font-weight:bold; white-space:nowrap;">'.esc_html(ABSPATH).'</td><td><input type="text" name="'.esc_attr(__NAMESPACE__).'[save_options][htmlc_cache_dir_private]" value="'.esc_attr(plugin()->options['htmlc_cache_dir_private']).'" /></td><td style="width:1px; font-weight:bold; white-space:nowrap;">/</td></tr></table>'."\n";
					echo '         <p class="notice" style="display:block;">'.__('<strong>Important:</strong> this should <strong>NOT</strong> be the same as your primary Quick Cache directory. These cache files should be given their own <strong>separate</strong> directory on the server. This is where the HTML Compressor will store any systematic cache files (for private internal use only) that are still fresh.', plugin()->text_domain).'</p>'."\n";
					echo '         <p><input type="text" name="'.esc_attr(__NAMESPACE__).'[save_options][htmlc_cache_expiration_time]" value="'.esc_attr(plugin()->options['htmlc_cache_expiration_time']).'" /></p>'."\n";
					...
					echo '      <table style="width:100%;"><tr><td style="width:1px; font-weight:bold; white-space:nowrap;">/cache/PROTOCOL/HOST/REQUEST_URI.</td><td><input type="text" name="'.esc_attr(__NAMESPACE__).'[save_options][version_salt]" value="" class="monospace" placeholder="$_COOKIE[\'my_cookie\']" /></td><td style="width:1px; font-weight:bold; white-space:nowrap;"></td></tr></table>'."\n";
