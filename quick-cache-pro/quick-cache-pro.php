					...
					public function admin_bar_menu(&$wp_admin_bar)
						{
							...
							$wp_admin_bar->add_node(array('parent' => 'top-secondary', 'id' => __NAMESPACE__, 'title' => __('Clear Cache', $this->text_domain), 'href' => '#', 'meta' => array('title' => __('Powered by Quick Cache!', $this->text_domain), 'class' => __NAMESPACE__, 'tabindex' => -1)));
						}
					...
					public function auto_purge_user_cache($user_id)
						{
							...
							$md5_4 = md5($user_id); // An MD5 hash in position 4; based on user ID.

							foreach((array)glob($cache_dir.'/qc-c-*-*-*-'.$md5_4, GLOB_NOSORT) as $_file) if($_file && is_file($_file))
								{
									if(!unlink($_file)) // If file deletion fails; stop here w/ exception.
										throw new \exception(sprintf(__('Unable to auto-purge: `%1$s`.', $this->text_domain), $_file));
									$counter++; // Increment counter for each file purge.
