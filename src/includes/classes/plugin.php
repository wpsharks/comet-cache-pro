...
class Plugin extends AbsBaseAp
{
    ...
    {
        if (!is_null($setup = &$self->staticKey(__FUNCTION__))) {
        ...
        $this->cap           = $this->apply_wp_filters(__METHOD__.'__cap', $this->cap);
        $this->update_cap    = $this->apply_wp_filters(__METHOD__.'__update_cap', $this->update_cap);
        $this->network_cap   = $this->apply_wp_filters(__METHOD__.'__network_cap', $this->network_cap);
        $this->uninstall_cap = $this->apply_wp_filters(__METHOD__.'__uninstall_cap', $this->uninstall_cap);
    ...
    public function auto_clear_post_cache($post_id, $force = false)
    {
        ...
        if (!empty($this->pre_post_update_post_permalink[$post_id]) && ($permalink = $this->pre_post_update_post_permalink[$post_id])) {
            $this->pre_post_update_post_permalink[$post_id] = '';
        } // Reset; only used for post status transitions
        ...
        if (($type = get_post_type($post_id)) && ($type = get_post_type_object($type)) && !empty($type->labels->singular_name)) {
            ...
            $type_singular_name = __('Post', $this->text_domain);
        } // Default value.
