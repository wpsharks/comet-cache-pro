...
class MenuPageOptions extends MenuPage
{
    ...
    public function __construct()
    {
        ...
        if (current_user_can($this->plugin->update_cap)) {
            echo '<a href="'.esc_attr(add_query_arg(urlencode_deep(array('page' => GLOBAL_NS.'-pro-updater')), self_admin_url('/admin.php'))).'"><i class="fa fa-magic"></i> '.__('Pro Updater', $this->plugin->text_domain).'</a>'."\n";
        }
        ...
        echo '         <table style="width:100%;"><tr><td><input type="text" name="'.esc_attr(GLOBAL_NS).'[saveOptions][auto_cache_user_agent]" value="'.esc_attr($this->plugin->options['auto_cache_user_agent']).'" /></td><td style="width:1px; font-weight:bold; white-space:nowrap;">; '.esc_html(GLOBAL_NS.' '.$this->plugin->version).'</td></tr></table>'."\n";
        ...
        echo '      <p>'.sprintf(__('Upload your <code>%1$s-options.json</code> file and click "Save All Changes" below. The options provided by your import file will override any that exist currently.', $this->plugin->text_domain), GLOBAL_NS).'</p>'."\n";
        ...
        echo '      <button type="button" class="plugin-menu-page-export-options" style="float:right; margin: 0 0 0 25px;"'.// Exports existing options from this installation.
             ...
             '         '.sprintf(__('%1$s-options.json', $this->plugin->text_domain), GLOBAL_NS).' <i class="fa fa-arrow-circle-o-down"></i></button>'."\n";
