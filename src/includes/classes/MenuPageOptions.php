...
class MenuPageOptions extends MenuPage
{
    ...
    public function __construct()
    {
        ...
        echo '      <p class="info">'.__('<strong>Tip:</strong> let\'s use this example URL: <code>http://www.example.com/post/example-post-123</code>. To exclude this URL, you would put this line into the field above: <code>/post/example-post-123</code>. Or, you could also just put in a small fragment, like: <code>example</code> or <code>example-*-123</code> and that would exclude any URI containing that word fragment. To exclude only the front page, put it in a fragment like: <code>^/$</code>. To exclude a category, e.g. <code>/blog/</code>, and only that category, use <code>^/blog/$</code>. To exclude all posts and its subdirectories, use a fragment like: <code>^/post/**</code> and that would exclude any URI in any directory inside <code>/post</code>. To exclude any URI that contains <code>/post/</code> anywhere, use a fragment like <code>/post/</code>.', SLUG_TD).'</p>'."\n";
