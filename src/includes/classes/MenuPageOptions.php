...
class MenuPageOptions extends MenuPage
{
    ...
    public function __construct()
    {
        ...
        if (IS_PRO || $this->plugin->isProPreview()) {
            ...
            echo '      <p><select name="'.esc_attr(GLOBAL_NS).'[saveOptions][cache_clear_admin_bar_enable]" data-target=".-cache-clear-admin-bar-roles-caps" style="width:auto;">'."\n";
        ...
        if (IS_PRO || $this->plugin->isProPreview()) {
            echo '  <div class="'.(!IS_PRO ? 'pro-preview' : '').'">'."\n";
            echo '      <hr />'."\n";
            echo '      <h3>'.__('Disable Cache Expiration If Server Load Average is High?', SLUG_TD).'</h3>'."\n";
            echo '      <p>'.sprintf(__('If you have high traffic at certain times of the day, %1$s can be told to check the current load average via <a href="http://zencache.com/r/system-load-average-via-php/" target="_blank"><code>sys_getloadavg()</code></a>. If your server\'s load average has been high in the last 15 minutes or so, cache expiration is disabled automatically to help reduce stress on the server; i.e., to avoid generating a new version of the cache while the server is very busy.', SLUG_TD), esc_html(NAME)).'</p>'."\n";
            echo '      <p>'.sprintf(__('To enable this functionality you should first determine what a high load average is for your server. If you log into your machine via SSH you can run the <code>top</code> command to get a feel for what a high load average looks like. Once you know the number, you can enter it in the field below; e.g., <code>1.05</code> might be a high load average for a server with one CPU. See also: <a href="http://zencache.com/r/understanding-load-average/" target="_blank">Understanding Load Average</a>', SLUG_TD), esc_html(NAME)).'</p>'."\n";
            echo '      <p><input type="text" name="'.esc_attr(GLOBAL_NS).'[saveOptions][cache_max_age_disable_if_load_average_is_gte]" value="'.esc_attr($this->plugin->options['cache_max_age_disable_if_load_average_is_gte']).'" /></p>'."\n";
            if (stripos(PHP_OS, 'win') === 0) { // See: <http://jas.xyz/1HZsZ9v>
                echo '  <p class="warning">'.__('<strong>Note:</strong> <code>sys_getloadavg()</code> is not implemented in PHP for Windows servers yet.', SLUG_TD).'</p>'."\n";
            }
            echo '   </div>'."\n";
        }
        echo '   </div>'."\n";
        ...
        if (IS_PRO || $this->plugin->isProPreview()) {
            ...
            echo '   <div class="plugin-menu-page-panel-body clearfix'.(!IS_PRO ? ' pro-preview' : '').'">'."\n";
            echo '      <i class="si si-cloud si-4x" style="float:right; margin: 0 0 0 25px;"></i>'."\n";
