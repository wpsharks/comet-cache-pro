...
class MenuPageOptions extends MenuPage
{
    ...
    public function __construct()
    {
        ...
        if (IS_PRO || $this->plugin->isProPreview()) {
            ...
            echo '      <p><select name="'.esc_attr(GLOBAL_NS).'[saveOptions][admin_bar_when_logged_in]">'."\n";
            echo '            <option value="0"'.selected($this->plugin->options['admin_bar_when_logged_in'], '0', false).'>'.__('No, leave the Admin Bar disabled for logged-in users.(recommended option)', SLUG_TD).'</option>'."\n";
            echo '            <option value="1"'.selected($this->plugin->options['admin_bar_when_logged_in'], 'postload', false).'>'.__('Yes, enable the Admin Bar for logged-in users.', SLUG_TD).'</option>'."\n";

            if ($this->plugin->options['admin_bar_when_logged_in'] === '0' && $this->plugin->applyWpFilters(GLOBAL_NS.'_when_logged_in_no_admin_bar', true)) {
                echo '<p class="warning">'.sprintf(__('<strong>Warning:</strong> Whenever you enable caching for logged-in users (without a separate cache for each user), the WordPress Admin Bar <em>must</em> be disabled to prevent one user from seeing another user\'s details in the Admin Bar. <strong>Given your current configuration, %1$s will automatically hide the WordPress Admin Bar on the front-end of your site.</strong>', SLUG_TD), esc_html(NAME)).'</p>'."\n";
            }
