...
class MenuPageOptions extends MenuPage
{
    ...
    public function __construct()
    {
        ...
        if (IS_PRO || $this->plugin->isProPreview()) {
            ...
            echo '         <p>'.__('Disabled by default. This setting is only applicable when caching for Logged-In Users is enabled. This should remain disabled for logged-in users because the user-specific cache has a much shorter Time To Live (TTL) than the cache for a normal visitor. The user-specific cache is likely to expire more quickly and frequently rebuilding the HTML Comperssor cache for a user will actually slow things down for them. For example, if you\'re logged into the site as a user and you submit a form, that triggers a clearing of the cache for that user (including the HTML Compressor cache). Lots of little actions you take can result in a clearing of the cache. This shorter TTL is not ideal when running the HTML Compressor. The HTML Compressor does a deep analysis of the page content and the associated resources in order to compress things for you intelligently. For logged-in users, it is better to skip that and just cache the HTML source as-is, avoiding that extra overhead. In short, do NOT turn this on unless you know what you\'re doing.', SLUG_TD).'</p>'."\n";
