...
class Memcached extends Classes\AbsBase
{
    ...
    public function __construct()
    {
        ...
        if (IS_PRO || $this->plugin->isProPreview()) {
            ...
            echo '            <option value="0"'.(!IS_PRO ? '' : selected($this->plugin->options['memcached_enable'], '0', false)).'>'.__('No, serve cache files from disk (default behavior).', SLUG_TD).'</option>'."\n";
            echo '            <option value="1"'.(!IS_PRO ? ' selected' : selected($this->plugin->options['memcached_enable'], '1', false)).'>'.__('Yes, serve cached pages from RAM via Memcached (recommended).', SLUG_TD).'</option>'."\n";
            echo '         </select></p>'."\n";

            echo '      <div class="plugin-menu-page-panel-if-enabled -memcached-options"><hr />'."\n";
            echo '        <h3>'.__('Memcached Server Pool', SLUG_TD).'</h3>'."\n";
