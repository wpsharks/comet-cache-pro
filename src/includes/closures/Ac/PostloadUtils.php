...
$self->postload = array(
    ...
    'wp_main_query'        => true,
    'set_debug_info'       => INTELLICACHE_DEBUGGING_ENABLE,
);
...
$self->maybePostloadInvalidateWhenLoggedIn = function () use ($self) {
    if (INTELLICACHE_WHEN_LOGGED_IN !== 'postload') {
    ...
    } elseif (!INTELLICACHE_GET_REQUESTS && $self->requestContainsUncacheableQueryVars()) {
        $self->postload['invalidate_when_logged_in'] = true;
    }
};
/*[/pro]*/
...
$self->maybeStartObWhenLoggedInPostload = function () use ($self) {
    ...
    $self->cache_file = INTELLICACHE_DIR.'/'.$self->cache_path; // Now considering a user token.
...
/*
 * Grab details from WP and the IntelliCache plugin itself,
