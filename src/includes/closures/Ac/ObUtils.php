 ...
 * @since 150422 Rewrite.
 *
 * @note This is a vital part of IntelliCache. This method serves existing (fresh) cache files.
...
$self->maybeStartOutputBuffering = function () use ($self) {
    ...
    if (defined('INTELLICACHE_ALLOWED') && !INTELLICACHE_ALLOWED) {
        return $self->maybeSetDebugInfo(NC_DEBUG_INTELLICACHE_ALLOWED_CONSTANT);
    }
    if (isset($_SERVER['INTELLICACHE_ALLOWED']) && !$_SERVER['INTELLICACHE_ALLOWED']) {
        return $self->maybeSetDebugInfo(NC_DEBUG_INTELLICACHE_ALLOWED_SERVER_VAR);
    }
    ...
    if (!INTELLICACHE_FEEDS_ENABLE && $self->isFeed()) {
    ...
    if ((!IS_PRO || !INTELLICACHE_WHEN_LOGGED_IN) && $self->isLikeUserLoggedIn()) {
        return $self->maybeSetDebugInfo(NC_DEBUG_IS_LIKE_LOGGED_IN_USER);
    }
    if (!INTELLICACHE_GET_REQUESTS && $self->requestContainsUncacheableQueryVars()) {
        return $self->maybeSetDebugInfo(NC_DEBUG_GET_REQUEST_QUERIES);
    }
    if (!empty($_REQUEST['preview'])) {
        return $self->maybeSetDebugInfo(NC_DEBUG_PREVIEW);
    }
    if (INTELLICACHE_EXCLUDE_URIS && preg_match(INTELLICACHE_EXCLUDE_URIS, $_SERVER['REQUEST_URI'])) {
        return $self->maybeSetDebugInfo(NC_DEBUG_EXCLUDED_URIS);
    }
    if (INTELLICACHE_EXCLUDE_AGENTS && !empty($_SERVER['HTTP_USER_AGENT']) && (!IS_PRO || !$self->isAutoCacheEngine())) {
        if (preg_match(INTELLICACHE_EXCLUDE_AGENTS, $_SERVER['HTTP_USER_AGENT'])) {
            return $self->maybeSetDebugInfo(NC_DEBUG_EXCLUDED_AGENTS);
        }
    }
    if (INTELLICACHE_EXCLUDE_REFS && !empty($_REQUEST['_wp_http_referer'])) {
        if (preg_match(INTELLICACHE_EXCLUDE_REFS, stripslashes($_REQUEST['_wp_http_referer']))) {
            return $self->maybeSetDebugInfo(NC_DEBUG_EXCLUDED_REFS);
        }
    }
    if (INTELLICACHE_EXCLUDE_REFS && !empty($_SERVER['HTTP_REFERER'])) {
        if (preg_match(INTELLICACHE_EXCLUDE_REFS, $_SERVER['HTTP_REFERER'])) {
            return $self->maybeSetDebugInfo(NC_DEBUG_EXCLUDED_REFS);
        }
    }
    ...
    $self->version_salt = INTELLICACHE_VERSION_SALT; // Initialize the version salt. /*[/pro]*/
    ...
    $self->cache_file = INTELLICACHE_DIR.'/'.$self->cache_path; // Not considering a user cache. That's done in the postload phase.

    $self->cache_path_404 = $self->buildCachePath($self->protocol.$self->host_token.rtrim($self->host_base_dir_tokens, '/').'/'.INTELLICACHE_404_CACHE_FILENAME);
    $self->cache_file_404 = INTELLICACHE_DIR.'/'.$self->cache_path_404; // Not considering a user cache at all here--ever.

    $self->salt_location = ltrim($self->version_salt.' '.$self->protocol.$self->host_token.$_SERVER['REQUEST_URI']);

    $self->cache_max_age = strtotime('-'.INTELLICACHE_MAX_AGE);
    /*[pro strip-from="lite"]*/ // Pro version allows for load average checks.
    if (INTELLICACHE_MAX_AGE_DISABLE_IF_LOAD_AVERAGE_IS_GTE && ($load_averages = $self->sysLoadAverages())) {
        if (max($load_averages) >= INTELLICACHE_MAX_AGE_DISABLE_IF_LOAD_AVERAGE_IS_GTE) {
            $self->cache_max_age = 0; // No expiration time.
        }
    } /*[/pro]*/
    if (IS_PRO && INTELLICACHE_WHEN_LOGGED_IN === 'postload' && $self->isLikeUserLoggedIn()) {
        ...
        if (INTELLICACHE_DEBUGGING_ENABLE && $self->isHtmlXmlDoc($cache)) {
...
$self->outputBufferCallbackHandler = function ($buffer, $phase) use ($self) {
    ...
    if (defined('INTELLICACHE_ALLOWED') && !INTELLICACHE_ALLOWED) {
        return (boolean) $self->maybeSetDebugInfo(NC_DEBUG_INTELLICACHE_ALLOWED_CONSTANT);
    }
    if (isset($_SERVER['INTELLICACHE_ALLOWED']) && !$_SERVER['INTELLICACHE_ALLOWED']) {
        return (boolean) $self->maybeSetDebugInfo(NC_DEBUG_INTELLICACHE_ALLOWED_SERVER_VAR);
    }
    if (defined('DONOTCACHEPAGE')) {
    ...
    if ((!IS_PRO || !INTELLICACHE_WHEN_LOGGED_IN) && $self->is_user_logged_in) {
    ...
    if (!INTELLICACHE_CACHE_NONCE_VALUES && preg_match('/\b(?:_wpnonce|akismet_comment_nonce)\b/', $cache)) {
        if (IS_PRO && INTELLICACHE_WHEN_LOGGED_IN && $self->isLikeUserLoggedIn()) {
            if (!INTELLICACHE_CACHE_NONCE_VALUES_WHEN_LOGGED_IN) {
    ...
    if ($self->is_404 && !INTELLICACHE_CACHE_404_REQUESTS) {
    ...
    if (!is_dir(INTELLICACHE_DIR) && mkdir(INTELLICACHE_DIR, 0775, true) && !is_file(INTELLICACHE_DIR.'/.htaccess')) {
        file_put_contents(INTELLICACHE_DIR.'/.htaccess', $self->htaccess_deny);
    }
    ...
    if ($self->is_404 && is_file($self->cache_file_404)) {
        if (!(symlink($self->cache_file_404, $cache_file_tmp) && rename($cache_file_tmp, $self->cache_file))) {
            throw new \Exception(sprintf(__('Unable to create symlink: `%1$s` » `%2$s`. Possible permissions issue (or race condition), please check your cache directory: `%3$s`.', SLUG_TD), $self->cache_file, $self->cache_file_404, INTELLICACHE_DIR));
        }
    ...
    if (INTELLICACHE_DEBUGGING_ENABLE && $self->isHtmlXmlDoc($cache)) {
        ...
        $cache .= "\n".'<!-- '.htmlspecialchars(sprintf(__('%1$s file built for (%2$s%3$s) in %4$s seconds, on: %5$s.', SLUG_TD), NAME, $self->is_404 ? '404 [error document]' : $self->salt_location, (IS_PRO && INTELLICACHE_WHEN_LOGGED_IN && $self->user_token ? '; '.sprintf(__('user token: %1$s', SLUG_TD), $self->user_token) : ''), $total_time, date('M jS, Y @ g:i a T'))).' -->';
        $cache .= "\n".'<!-- '.htmlspecialchars(sprintf(__('This %1$s file will auto-expire (and be rebuilt) on: %2$s (based on your configured expiration time).', SLUG_TD), NAME, date('M jS, Y @ g:i a T', strtotime('+'.INTELLICACHE_MAX_AGE)))).' -->';
    }
    if ($self->is_404) {
    ...
    throw new \Exception(sprintf(__('%1$s: failed to write cache file for: `%2$s`; possible permissions issue (or race condition), please check your cache directory: `%3$s`.', SLUG_TD), NAME, $_SERVER['REQUEST_URI'], INTELLICACHE_DIR));
};
