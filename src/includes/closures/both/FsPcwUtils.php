...
$self->deleteFilesFromCacheDir = function ($regex, $check_max_age = false) use ($self) {
    ...
    if ($check_max_age && (empty($self->options) || !is_array($self->options) || !isset($self->options['cache_max_age']))) {
        throw new \Exception(__('The `options` property w/ a `cache_max_age` key is not defined in this class.', $self->text_domain));
    }
