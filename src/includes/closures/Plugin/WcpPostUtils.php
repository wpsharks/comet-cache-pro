...
$self->autoClearPostCache = function ($post_id, $force = false) use ($self) {
    ...
    if (!is_null($done = &$self->cacheKey('autoClearPostCache', [$post_id, $force]))) {
...
$self->autoClearPostCacheTransition = function ($post_id, $data) use ($self) {
    ...
    if (!is_null($done = &$self->cacheKey('autoClearPostCacheTransition', [$old_status, $new_status, $post_id]))) {
        return $counter; // Already did this.
    }
    $done = true; // Flag as having been done.
