 ...
 *    existing set of website files (like the AWS Elastic Beanstalk does) will NOT cause IntelliCache
 *    to stop functioning due to the lack of an `advanced-cache.php` file, which is generated by IntelliCache.
 *
 ...
 *    IntelliCache creates this file on its own. Thus, if it's missing (and QC is active)
 *    we simply regenerate the file automatically to keep IntelliCache running.
 */
$self->checkAdvancedCache = function () use ($self) {
    ...
    //      See: <https://github.com/websharks/intellicache/issues/432>

    // Also fixes a missing `define('WP_CACHE', TRUE)` bug related to migrating from Quick Cache
    //      See <https://github.com/websharks/intellicache/issues/450>
...
$self->addAdvancedCache = function () use ($self) {
    ...
    // Ignore; this is created by IntelliCache; and we don't need to obey in this case.
    #if(defined('DISALLOW_FILE_MODS') && DISALLOW_FILE_MODS)
...
$self->removeAdvancedCache = function () use ($self) {
    ...
    /* Empty the file only. This way permissions are NOT lost in cases where
        a site owner makes this specific file writable for IntelliCache. */
 ...
 * @note This runs so that remote deployments which completely wipe out an
 *    existing set of website files (like the AWS Elastic Beanstalk does) will NOT cause IntelliCache
 *    to stop functioning due to the lack of a `[SHORT_NAME]-blog-paths` file, which is generated by IntelliCache.
 *
 *    For instance, if you have a Git repo with all of your site files; when you push those files
