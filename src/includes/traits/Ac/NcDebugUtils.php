        ...
        if (!$reason) {
            switch ($reason_code) {
                ...
                case $this::NC_DEBUG_XMLRPC_REQUEST_CONSTANT:
                    $reason = __('because the PHP constant `XMLRPC_REQUEST` has been set to a boolean-ish `TRUE` value at runtime. XML-RPC requests are never cached, as they are often very dynamic in nature.', SLUG_TD);
                    break; // Break switch handler.

                case $this::NC_DEBUG_REST_REQUEST_CONSTANT:
                    $reason = __('because the PHP constant `REST_REQUEST` has been set to a boolean-ish `TRUE` value at runtime. REST requests are never cached, as they are often very dynamic in nature.', SLUG_TD);
                    break; // Break switch handler.
                ...
                case $this::NC_DEBUG_IS_LOGGED_IN_USER_NONCE:
                    $reason = __('because the current page contains `_wpnonce` or `akismet_comment_nonce`. While your current configuration states that pages should be cached for logged-in visitors, it also states that pages with dynamic `*nonce*` values in the markup should not be cached, even for logged-in visitors. See http://wsharks.com/1O1Kudy for further details.', SLUG_TD);
                    break; // Break switch handler.

                case $this::NC_DEBUG_PAGE_CONTAINS_NONCE:
                    $reason = __('because the current page contains `_wpnonce` or `akismet_comment_nonce`. Your current configuration states that pages with dynamic `*nonce*` values in the markup should not be cached. See http://wsharks.com/1O1Kudy for further details.', SLUG_TD);
                    break; // Break switch handler.
