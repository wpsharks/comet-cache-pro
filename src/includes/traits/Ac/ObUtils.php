...
trait ObUtils
{
     ...
     */
    public $mobile_adaptive_salt = ''; /*[/pro]*/
    ...
    public function maybeStartOutputBuffering()
    {
        ...
        /*[pro strip-from="lite"]*/ // Allow salt options in the pro version.
        if (COMET_CACHE_MOBILE_ADAPTIVE_SALT_ENABLE && COMET_CACHE_MOBILE_ADAPTIVE_SALT && $this->uaIsMobile()) {
            if (($this->mobile_adaptive_salt = $this->fillUaTokens(COMET_CACHE_MOBILE_ADAPTIVE_SALT))) {
                $this->version_salt = 'ua='.$this->mobile_adaptive_salt.';';
            } // Anything that ends up being empty has no UA salt.
        }
        // Fill the version salt in pro version.
        $this->version_salt .= (string) COMET_CACHE_VERSION_SALT; /*[/pro]*/
        ...
        if (COMET_CACHE_MAX_AGE_DISABLE_IF_LOAD_AVERAGE_IS_GTE && ($load_averages = $this->sysLoadAverages())) {
            if (max($load_averages) >= COMET_CACHE_MAX_AGE_DISABLE_IF_LOAD_AVERAGE_IS_GTE) {
                $this->cache_max_age_disabled = true; // No expiration time.
            }
        } /*[/pro]*/
