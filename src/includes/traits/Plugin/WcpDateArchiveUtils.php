...
trait WcpDateArchiveUtils
{
    ...
    public function autoClearDateArchiveCache($post_id, $force = false)
    {
        ...
        if ($this->options['cache_clear_date_archives_enable'] === '1') {
            $date_archive_links[sprintf(__('%1$s Date Archive', SLUG_TD), $Y)]                   = get_year_link($Y);
            $date_archive_links[sprintf(__('%1$s/%2$s Date Archive', SLUG_TD), $Y, $m)]          = get_month_link($Y, $m);
            $date_archive_links[sprintf(__('%1$s/%2$s/%3$s Date Archive', SLUG_TD), $Y, $m, $j)] = get_day_link($Y, $m, $j);
        ...
        foreach ($date_archive_urls as $_label => $_url) {
            ...
            if ($_url_counter && $enqueued_notices < 100 && is_admin() && (!IS_PRO || $this->options['change_notifications_enable'])) {
                $this->enqueueNotice(sprintf(__('Found %1$s in the cache for %2$s; auto-clearing.', SLUG_TD), esc_html($this->i18nFiles($_url_counter)), esc_html($_label)), ['combinable' => true]);
