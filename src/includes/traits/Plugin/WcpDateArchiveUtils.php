...
trait WcpDateArchiveUtils
{
    ...
    public function autoClearDateArchiveCache($post_id, $force = false)
    {
        ...
        if ($this->options['cache_clear_date_archives_enable'] === '1') {
            $date_archive_urls[sprintf(__('%1$s Date Archive', SLUG_TD), $Y)]                   = get_year_link($Y);
            $date_archive_urls[sprintf(__('%1$s/%2$s Date Archive', SLUG_TD), $Y, $m)]          = get_month_link($Y, $m);
            $date_archive_urls[sprintf(__('%1$s/%2$s/%3$s Date Archive', SLUG_TD), $Y, $m, $j)] = get_day_link($Y, $m, $j);
