...
/*![pro strip-from='lite']*/
b("#"+a.namespace+"-mobile-adaptive-salt",a.$menuPage).on("blur",a.validateMobileAdaptiveSalt);a.$menuPage.on("submit",a.validateMobileAdaptiveSalt);
/*![/pro]*/
/*![pro strip-from='lite']*/
...
/*![/pro]*/
/*![pro strip-from='lite']*/
(function(){if(b(".plugin-menu-page-stats",a.$menuPage).length){b(".plugin-menu-page-stats-button").on("click",a.statsRefresh);a.stats()}})();
/*![/pro]*/
...
/*![pro strip-from='lite']*/
a.validateMobileAdaptiveSalt=function(i){var h=["os.name","device.type","browser.name"];var k=["os.name","device.type","device.is_mobile","browser.name","browser.version","browser.version.major"];var e=b("#"+a.namespace+"-mobile-adaptive-salt",a.$menuPage);var g,f,j;if(e.length&&(g=b.trim(e.val()))){for(j=0,f=g.split(/[+\s]+/);j<f.length;j++){if(b.inArray(f[j],k)===-1){if(i&&i.type==="submit"){e.val(h.join(" + "))}else{alert(a.vars.i18n.mobileAdaptiveSaltError)}return}}}};
/*![/pro]*/
