...
/*![pro strip-from="lite"]*/
b("#wp-admin-bar-"+a.namespace+"-clear-options-wrapper .-home-url-only > a").on("click",a.clearCacheHomeUrlOnly);b("#wp-admin-bar-"+a.namespace+"-clear-options-wrapper .-current-url-only > a").on("click",a.clearCacheCurrentUrlOnly);b("#wp-admin-bar-"+a.namespace+"-clear-options-wrapper .-specific-url-only > a").on("click",a.clearCacheSpecificUrlOnly);b("#wp-admin-bar-"+a.namespace+"-clear-options-wrapper .-opcache-only > a").on("click",a.clearCacheOpCacheOnly);b("#wp-admin-bar-"+a.namespace+"-clear-options-wrapper .-cdn-only > a").on("click",a.clearCacheCdnOnly);b("#wp-admin-bar-"+a.namespace+"-clear-options-wrapper .-transients-only > a").on("click",a.clearExpiredTransientsOnly);
/*![/pro]*/
c.on("click","."+a.namespace+"-ajax-response",a.hideAJAXResponse);
/*![pro strip-from="lite"]*/
var f=b("#wp-admin-bar-"+a.namespace+"-stats"),e=f.find(".-wrapper"),g=e.find(".-container");
/*![/pro]*/
/*![pro strip-from="lite"]*/
(function(){if(f.length&&a.MutationObserver){(new a.MutationObserver(function(h){b.each(h,function(k,i){if(i.type!=="attributes"){return}if(i.attributeName!=="class"){return}var j=i.oldValue,l=b(i.target).prop(i.attributeName);if(!/\bhover\b/i.test(j)&&/\bhover\b/i.test(l)){a.stats()}return false})})).observe(f[0],{attributes:true,attributeOldValue:true,childList:true,characterData:true})}})();
/*![/pro]*/
...
/*![/pro]*/
/*![pro strip-from="lite"]*/
var l=b.extend({},{urlOnly:"",opCacheOnly:false,cdnOnly:false,transientsOnly:false},f||{});
/*![/pro]*/
var e={_wpnonce:a.vars._wpnonce};var i=false,k,h=b(),g=b();
/*![pro strip-from="lite"]*/
if(l.urlOnly){i=true;e[a.namespace]={ajaxClearCacheUrl:l.urlOnly}}else{if(l.opCacheOnly){i=true;e[a.namespace]={ajaxClearOpCache:"1"}}else{if(l.cdnOnly){i=true;e[a.namespace]={ajaxClearCdnCache:"1"}}else{if(l.transientsOnly){i=true;e[a.namespace]={ajaxClearExpiredTransients:"1"}}else{
/*![/pro]*/
...
/*![pro strip-from="lite"]*/
h=b("#wp-admin-bar-"+a.namespace+"-clear-options-wrapper .-label");g=b("#wp-admin-bar-"+a.namespace+"-clear-options-wrapper  .-options");
/*![/pro]*/
...
/*![pro strip-from="lite"]*/
a.clearCacheHomeUrlOnly=function(e){a.clearCache(e,{urlOnly:"home"})};
/*![/pro]*/
/*![pro strip-from="lite"]*/
a.clearCacheCurrentUrlOnly=function(e){a.clearCache(e,{urlOnly:document.URL})};
/*![/pro]*/
/*![pro strip-from="lite"]*/
a.clearCacheSpecificUrlOnly=function(f){var e=b.trim(prompt(a.vars.i18n.enterSpecificUrl,"http://"));if(e&&e!=="http://"){a.clearCache(f,{urlOnly:e})}else{a.preventDefault(f)}};
/*![/pro]*/
/*![pro strip-from="lite"]*/
a.clearCacheOpCacheOnly=function(e){a.clearCache(e,{opCacheOnly:true})};
/*![/pro]*/
/*![pro strip-from="lite"]*/
a.clearCacheCdnOnly=function(e){a.clearCache(e,{cdnOnly:true})};
/*![/pro]*/
/*![pro strip-from="lite"]*/
a.clearExpiredTransientsOnly=function(e){a.clearCache(e,{transientsOnly:true})};
/*![/pro]*/
